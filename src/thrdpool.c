...
static void *__thrdpool_routine(void *arg)
{
    ...
    if (memcmp(&tid, &__zero_tid, sizeof(pthread_t)) != 0) pthread_join(tid, NULL);
...
static void __thrdpool_terminate(int in_pool, thrdpool_t *pool)
{
    ...
    pthread_mutex_unlock(&pool->mutex);
    if (memcmp(&pool->tid, &__zero_tid, sizeof(pthread_t)) != 0) pthread_join(pool->tid, NULL);
}
...
int thrdpool_in_pool(thrdpool_t *pool) { return pthread_getspecific(pool->key) == pool; }
