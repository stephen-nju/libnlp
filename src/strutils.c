...
nlp_uint8_t *utf8str_str(const nlp_uint8_t *haystack, const nlp_uint8_t *needle) {
    ...
    /* Is needle empty?  */
    if (first == 0) return (nlp_uint8_t *)haystack;

    /* Is needle nearly empty (only one unit)?  */
    if (needle[1] == 0) return utf8str_chr(haystack, first);


    return (nlp_uint8_t *)strstr((const char *)haystack, (const char *)needle);
    // #else
    //   {
